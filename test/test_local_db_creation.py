...
def test_alice_order(local_db_conn):
    ...
    assert (
        alice_order == expected_items
    ), f"Expected {expected_items}, found {alice_order}"
...
def test_charlie_reviews(local_db_conn):
    ...
    assert (
        charlie_reviews == expected_reviews
    ), f"Expected {expected_reviews}, found {charlie_reviews}"
