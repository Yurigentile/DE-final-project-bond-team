...
import json
from lambda_transform.src.load_new_data import load_new_data, retrive_list_of_files
...
class TestLoadNewDataWithoutMoto(unittest.TestCase):
    @patch("lambda_transform.src.load_new_data.boto3.client")
    def test_load_new_data(self, mock_boto_client):
        ...
        mock_file_list = [f"{timestamp}/design.json", f"{timestamp}/sales_order.json"]
        ...
        mock_s3.get_object.side_effect = lambda Bucket, Key: {
            "Body": MagicMock(
                read=lambda: json.dumps(
                    dataset_stub[Key.split("/")[-1].split(".")[0]]
                ).encode("utf-8")
            )
        }
