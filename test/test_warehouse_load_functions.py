...
from load_lambda.src.warehouse_load_functions import (
    get_secret,
...
class TestGetSecret(unittest.TestCase):
    ...
    def test_get_secret_success(self):
        ...
        self.assertEqual(
            result, self.secret_value, "Secret value does not match expected"
        )
...
class TestAlchemyCloseConnection(unittest.TestCase):
    ...
    def test_close_connection_exception(self, mock_logger):
        ...
        mock_logger.error.assert_called_with(
            "Error closing engine connection: Dispose failed"
        )
...
class TestLoadDataIntoWarehouse(unittest.TestCase):
    ...
    def test_load_data_into_warehouse_failure(self, mock_logger, mock_db_connection):
        ...
        with patch.object(
            pd.DataFrame, "to_sql", side_effect=Exception("Database error")
        ):
            # Act
            load_data_into_warehouse(dataframes, db_params)
            ...
            mock_logger.error.assert_any_call(
                "Failed to load table table1: Database error"
            )
            mock_logger.error.assert_any_call(
                "Failed to load table table2: Database error"
            )
